...
public class PolyUtil {
	 ...
	 */
	private static double tanLatGC(double lat1, double lat2, double lng2,
			double lng3) {
	 ...
	 * (lat2, lng2). lng1==0.
	 */
	private static double mercatorLatRhumb(double lat1, double lat2,
			double lng2, double lng3) {
	...
	private static boolean intersects(double lat1, double lat2, double lng2,
		...
		// as convenient.
		return geodesic ? tan(lat3) >= tanLatGC(lat1, lat2, lng2, lng3)
				: MathUtil.mercator(lat3) >= mercatorLatRhumb(lat1, lat2, lng2,
						lng3);
	}
	...
	public static boolean containsLocation(LatLng point, List<LatLng> polygon,
		...
		for (LatLng point2 : polygon) {
			...
			if (intersects(lat1, lat2, MathUtil.wrap(lng2 - lng1, -PI, PI),
					lat3, dLng3, geodesic)) {
				++nIntersect;
			}
			lat1 = lat2;
	 ...
	 */
	public static boolean isLocationOnEdge(LatLng point, List<LatLng> polygon,
	...
	public static boolean isLocationOnPath(LatLng point, List<LatLng> polyline,
			boolean geodesic, double tolerance) {
		return isLocationOnEdgeOrPath(point, polyline, false, geodesic,
				tolerance);
	}
	...
	private static boolean isLocationOnEdgeOrPath(LatLng point,
			List<LatLng> poly, boolean closed, boolean geodesic,
			...
			for (LatLng point2 : poly) {
				...
				if (max(lat1, lat2) >= minAcceptable
					...
					for (double x3 : xTry) {
						...
						double t = len2 <= 0 ? 0 : MathUtil.clamp(
								(x3 * x2 + (y3 - y1) * dy) / len2, 0, 1);
						double xClosest = t * x2;
						double yClosest = y1 + t * dy;
						double latClosest = MathUtil.inverseMercator(yClosest);
						double havDist = MathUtil.havDistance(lat3, latClosest,
								x3 - xClosest);
	 ...
	 */
	private static double sinDeltaBearing(double lat1, double lng1,
			double lat2, double lng2, double lat3, double lng3) {
	...
	private static boolean isOnSegmentGC(double lat1, double lng1, double lat2,
			double lng2, double lat3, double lng3, double havTolerance) {
		...
		double sinSumAlongTrack = MathUtil.sinSumFromHav(havAlongTrack13,
				havAlongTrack23);
